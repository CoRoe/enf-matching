...
def load_wav(fpath):
    ...
    with wave.open(fpath) as wav_f:
        ...
        #print(f"Loaded {fpath}, n_samples={len(data)} len_s={clip_len_s}")
        print(f"File {fpath}: Sample freqeuncy {fs} Hz, duration {clip_len_s} seconds")
...
def search(target_enf, reference_enf):
    ...
    :returns: list of tuples of (reference index, PMCC), sorted desc by PMCC
    """
    print(len(target_enf), len(reference_enf))
...
def gb_reference_data(year, month, day=None):
    ...
    if not os.path.exists(cache_fpath):
        ...
        ret_data = ret.json()['result']
        try:
            csv_resource = next(r for r in ret_data['resources'] if r['path'].endswith(f"/f-{year}-{month}.csv"))
...
def enf_match(fn, ref_fn):
    """ Matches an recorded audio file against a reference file with 50 Hz line
    noise. Both files must be in WAV format.

    :param: ref_fn: WAV file with 50 Hz line noise; sampiing frequency 400 HZ
    :param: fn:  WAV file with recorded audio
    """
    nominal_freq = 50
    freq_band_size = 0.2

    # !!!: make sure to run ./bin/download-example-files first
    ref_data, ref_fs = load_wav(ref_fn)
...
def match_timed(fn, year, month):
    gb_reference_data(year, month)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog='ENF matching')
    parser.add_argument('file')
    parser.add_argument('-r', '--ref', dest='referencefile')
    parser.add_argument('-y', '--year', dest='year')
    parser.add_argument('-m', '--month', dest='month')
    parser.add_argument('-v', '--verbose', action='store_true')
    args = parser.parse_args()
    if args.referencefile:
        enf_match(args.file, args.referencefile)
    else:
        # If  o reference file specified then year and month must be given
        if args.year and args.month:
            match_timed(args.file, args.year, args.month)
        else:
            print("Error: Both year and month must be specified")
